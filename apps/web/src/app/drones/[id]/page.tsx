...
import { deleteDrone, updateDrone } from "@/server/drones/actions";
  ...
  try {
    ...
    if (!record) {
      return (
        <PageShell>
          <DetailPanel
            title="Dron no encontrado"
            description="El dron solicitado no existe o ya no está disponible."
          >
            <p className="text-sm text-slate-500 dark:text-slate-400">
              Volvé al listado y seleccioná un dron válido.
            </p>
          </DetailPanel>
        </PageShell>
      );
      ...
      <PageShell>
        <div className="space-y-6">
          ...
          <DetailPanel
            ...
            description="Planes donde este dron está asignado."
          >
              ...
              {record.flightPlans.length > 0 ? (
                record.flightPlans.map((flightPlan) => (
                  <a
                    ...
                    className="block rounded-lg border border-slate-200 dark:border-slate-800/80 bg-white dark:bg-slate-950/45 px-4 py-3 transition hover:bg-slate-50 dark:hover:border-accent/40 dark:hover:bg-cyan-500/5"
                  >
                ...
                <p className="text-sm text-slate-500">
                  Este dron todavía no tiene planes de vuelo vinculados.
                </p>
              )}
            </div>
          </DetailPanel>
          ...
          <DroneForm
            ...
            initialValues={{
              ...
              insuranceExpiry: record.insuranceExpiry
                ? record.insuranceExpiry.toISOString().slice(0, 10)
                : "",
              notes: record.notes ?? "",
          ...
          <DetailPanel
            ...
            description="La eliminación lógica oculta este dron sin perder su historial."
          >
