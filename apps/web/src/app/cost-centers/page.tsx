...
export default async function CostCentersPage({
  ...
  await requirePageAuth(
    queryParams.toString() ? `/cost-centers?${queryParams.toString()}` : "/cost-centers",
  );

  try {
    return (
      <ListPage config={costCenterListConfig} fetchData={listCostCenters} searchParams={params} />
    );
  } catch (error) {
      ...
      <div className="p-6">
        <div className="rounded-xl border border-slate-200 dark:border-slate-800/80 bg-white dark:bg-slate-950/50 p-6">
          <h2 className="text-lg font-semibold text-slate-900 dark:text-white">
            Centros de costo no disponibles
          </h2>
