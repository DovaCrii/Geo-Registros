        ...
        <div key={event.id} className="relative flex gap-4 pb-6">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-sm font-medium text-slate-800 dark:text-white">
              <Link
                href={`/flight-plans/${event.flightPlan.id}`}
...
function getNextAction(stats: Awaited<ReturnType<typeof getDashboardStats>>): NextAction {
  if (stats.flightPlans.total === 0) {
    return {
      ...
      description:
        "Empezá cargando una misión para activar el tablero operativo y desbloquear el resto del flujo.",
  ...
  if (stats.pending.noGeometry > 0) {
    return {
      ...
      description:
        "Hay planes sin geometría. Abrí el editor de área para completar el mapa antes de revisar permisos.",
      href: "/flight-plans",
      cta: "Revisar planes",
    };
  }
  if (stats.pending.missingDocuments > 0) {
    return {
      ...
      description:
        "Faltan respaldos en algunos planes. Revisá documentos antes de enviar o cerrar el flujo.",
      href: "/flight-plans",
      cta: "Ver documentos",
    };
  }
  ...
  if (stats.pending.inReview > 0) {
    return {
      ...
      description:
        "Los planes en revisión necesitan seguimiento operativo antes de la siguiente transición.",
      href: "/flight-plans",
      cta: "Abrir revisión",
    };
  }
  if (stats.pending.upcomingFlights > 0) {
    return {
      ...
      description:
        "Hay vuelos agendados en los próximos días. Confirmá flota, operador y documentos.",
      href: "/flight-plans",
      cta: "Ver agenda",
    };
  }
  return {
    ...
    description:
      "No hay bloqueos inmediatos. Usá este momento para revisar vigencias y actualizar registros.",
    href: "/dashboard",
...
function getWorkflowStages(stats: Awaited<ReturnType<typeof getDashboardStats>>): WorkflowStage[] {
  ...
  const activeStage =
    stats.flightPlans.total === 0 ? 1 : stage2Active ? 2 : stage3Active ? 3 : stage4Active ? 4 : 5;

  return [
    {
      step: 1,
      title: "Crear misión",
      description: "Arrancá con un plan de vuelo para activar el flujo.",
      href: "/flight-plans/new",
      state: activeStage === 1 ? "active" : ("done" as const),
    },
    {
      step: 2,
      title: "Definir zona",
    ...
    },
    {
      ...
      description: "Seguimiento, observaciones y cierre operativo.",
      href: "/flight-plans",
  ...
  return [
    {
      ...
      description: "Definí geometría antes de enviar a revisión.",
      href: "/flight-plans",
    ...
    },
    {
      ...
      description: "Revisá respaldos para evitar bloqueos.",
      href: "/flight-plans",
    ...
    },
    {
      ...
      description: "Corregí observaciones para volver a avanzar.",
      href: "/flight-plans",
    ...
    },
    {
      ...
      description: "Dales seguimiento antes de pasar a autorización.",
      href: "/flight-plans",
    ...
    },
    {
      ...
      description: "Confirmá flota, operador y documentos.",
      href: "/flight-plans",
    ...
    },
    {
      ...
      description: "Completar licencias evita fricciones.",
      href: "/operators",
    ...
    },
    {
      ...
      description: "Mantené seguros y vencimientos listos.",
      href: "/drones",
  ...
  return (
    <div className="rounded-xl border border-slate-200 dark:border-slate-800/80 bg-white dark:bg-slate-950/45 p-6 shadow-sm dark:shadow-xl dark:shadow-slate-950/10">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.26em] text-accent dark:text-cyan-300">
            {action.label}
          </p>
        ...
        <span className="rounded-full border border-accent/20 dark:border-cyan-200 bg-accent/5 dark:bg-cyan-50 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.18em] text-accent dark:text-cyan-700">
          Guía
        </span>
      </div>
      ...
      <div className="mt-5 flex flex-wrap items-center gap-3">
        <Link
          href={action.href}
          className="inline-flex items-center justify-center rounded-lg border border-accent/30 bg-accent px-4 py-2.5 text-sm font-medium text-white transition hover:bg-accent-strong"
        >
          {action.cta}
        </Link>
    ...
    <div className="rounded-xl border border-slate-200 dark:border-slate-800/80 bg-white dark:bg-slate-950/45 p-6 shadow-sm dark:shadow-xl dark:shadow-slate-950/10">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <h2 className="mt-2 text-xl font-semibold text-slate-900 dark:text-white">
            Orden de trabajo más intuitivo
          </h2>
        </div>
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {stages.map((stage) => {
          ...
          const dotClasses =
            stage.state === "active"
              ? "bg-accent dark:bg-cyan-500"
              : stage.state === "done"
                ? "bg-success dark:bg-emerald-500"
                : "bg-slate-300 dark:bg-slate-600";
          return (
            <Link
              key={stage.step}
              href={stage.href}
              className={`rounded-xl border p-4 transition hover:-translate-y-0.5 hover:shadow-md ${stateClasses}`}
            >
...
function PriorityCard({ item }: { item: PriorityItem }) {
  const toneMap = {
    neutral:
      "border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950/70 text-slate-900 dark:text-slate-200",
    info: "border-accent/30 dark:border-cyan-200 bg-accent/5 dark:bg-cyan-50/90 text-accent-strong dark:text-cyan-950",
    warning:
      "border-status-warning/30 dark:border-amber-200 bg-status-warning/5 dark:bg-amber-50/90 text-status-warning dark:text-amber-950",
    danger:
      "border-status-danger/30 dark:border-rose-200 bg-status-danger/5 dark:bg-rose-50/90 text-status-danger dark:text-rose-950",
  } as const;

  return (
    <Link
      href={item.href}
      className={`rounded-xl border p-5 shadow-sm dark:shadow-lg transition hover:-translate-y-0.5 hover:shadow-md ${toneMap[item.tone]}`}
    >
      <div className="flex items-start justify-between gap-3">
        <div>
...
function NoPriorityState() {
  return (
    <div className="rounded-xl border border-success/30 dark:border-emerald-200 bg-success/5 dark:bg-emerald-50/80 p-6 shadow-sm dark:shadow-lg">
      <p className="text-xs font-semibold uppercase tracking-[0.26em] text-success dark:text-emerald-700">
        Todo al día
      </p>
      <h3 className="mt-2 text-lg font-semibold text-slate-900 dark:text-white">
        No hay pendientes operativos ahora mismo
      </h3>
      <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-400">
        Aprovechá este momento para revisar vigencias, preparar la próxima misión o dejar listo el
        siguiente plan.
      </p>
    </div>
  );
  ...
  return (
    <PageShell>
      ...
      <div className="space-y-6">
        {/* Header */}
        <div className="rounded-xl border border-slate-200 dark:border-slate-800/80 bg-white dark:bg-slate-950/45 p-6 shadow-sm dark:shadow-xl dark:shadow-slate-950/10">
          <p className="mb-1 text-xs font-semibold uppercase tracking-[0.28em] text-accent dark:text-cyan-300">
            Panel operativo
          </p>
          ...
          <p className="mt-1 max-w-2xl text-sm leading-6 text-slate-600 dark:text-slate-400">
            Centro operativo de AeroFlow: seguí el flujo recomendado, resolvé pendientes por impacto
        ...
        {params.error === "access-denied" && (
          <AlertCard
            ...
            message="No tenés permisos suficientes para acceder a esa sección. Si creés que esto es un error, contactá al administrador."
          />
        )}
          ...
          <AlertCard
            ...
            action={
              <div className="flex flex-wrap gap-2">
                <Link
                  href="/dashboard"
                  className="rounded-lg border border-status-warning/30 bg-status-warning/10 px-3 py-1.5 text-xs font-medium text-status-warning dark:text-amber-200 transition hover:border-status-warning/50 hover:bg-status-warning/20"
                >
                  Reintentar
                </Link>
                <Link
                  href="/flight-plans/new"
                  className="rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 px-3 py-1.5 text-xs font-medium text-slate-700 dark:text-slate-300 transition hover:bg-slate-50 dark:hover:bg-slate-700"
                >
        ...
        {stats.flightPlans.total === 0 ? (
          <EmptyState
            ...
            steps={[
              {
                number: 1,
                label: "Registrá un dron",
                description:
                  "Antes de volar, necesitás tener al menos un dron activo en el sistema.",
              },
              {
                number: 2,
                label: "Registrá un operador",
                description: "Cada plan de vuelo necesita un operador RPA responsable.",
              },
              {
                number: 3,
                label: "Creá un plan de vuelo",
                description: "Completá los datos básicos y empezá a operar.",
              },
            ]}
          />
        ) : null}
              ...
              <div className="mb-5 flex flex-wrap items-end justify-between gap-4">
                <div>
                  ...
                  <h2 className="mt-1 text-lg font-semibold text-slate-900 dark:text-white">
                    Ordenados por impacto en el vuelo
                  </h2>
                </div>
                    ...
                    {stats.expiring.drones.map((drone) => {
                      ...
                      return (
                        <Link
                          ...
                          className="flex items-center justify-between rounded-lg border border-slate-200 dark:border-amber-800/40 bg-white dark:bg-amber-950/50 px-4 py-3 text-sm transition hover:bg-slate-50 dark:hover:border-amber-600/40 dark:hover:bg-amber-900/50"
                        >
                          <span className="text-slate-700 dark:text-amber-100">
                            Seguro · {drone.code ?? drone.model}
                          </span>
                          <span
                            className={`text-xs font-medium ${expired ? "text-status-danger dark:text-red-400" : "text-status-warning dark:text-amber-300"}`}
                          >
                  ...
                  {stats.flightPlans.total > 0 ? (
                    <div className="flex flex-wrap gap-2">
                      {Object.entries(stats.flightPlans.byStatus).map(([status, count]) => (
                        <div
                          key={status}
                          className="flex items-center gap-2 rounded-lg border border-slate-200 dark:border-slate-800/80 bg-slate-50 dark:bg-slate-950/55 px-3 py-2"
                        >
                          <StatusBadge
                            status={STATUS_BADGE[status] ?? "planned"}
                    ...
                    <p className="text-sm text-slate-500">
                      ...
                      <Link
                        href="/flight-plans/new"
                        className="text-accent dark:text-cyan-300 hover:text-accent-strong dark:hover:text-cyan-200"
                      >
                        Creá el primero
                      </Link>
                    </p>
              ...
              <div className="space-y-6">
                ...
                <div className="rounded-xl border border-slate-200 dark:border-slate-800/80 bg-white dark:bg-slate-950/45 p-6 shadow-sm dark:shadow-xl dark:shadow-slate-950/10">
                  ...
                  <div className="space-y-3">
                    <Link
                      href="/flight-plans/new"
                      className="flex items-center justify-between rounded-lg border border-slate-200 dark:border-slate-800/80 bg-slate-50 dark:bg-slate-950/70 px-4 py-3 text-sm transition hover:bg-slate-100 dark:hover:border-accent/30 dark:hover:bg-slate-800/70"
                    >
                  ...
                  {stats.recentDocuments.length > 0 ? (
                    <div className="space-y-3">
                      {stats.recentDocuments.map((doc) => (
                        <div
                          key={doc.id}
                          className="rounded-lg border border-slate-200 dark:border-slate-800/80 bg-slate-50 dark:bg-slate-950/70 px-4 py-3"
                        >
