...
function WeatherDisplay({ data }: { data: WeatherData }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Min/Max labels */}
      <div className="-mt-3 flex justify-between px-4 text-[10px] uppercase tracking-wide text-slate-600 dark:text-slate-500">
        <span>Mín</span>
        <span>Máx</span>
      </div>
...
function WeatherErrorDisplay({ error }: { error: string }) {
  return (
    <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 dark:border-amber-500/20 dark:bg-amber-500/5">
      <p className="text-xs text-amber-700 dark:text-amber-300/80">{error}</p>
