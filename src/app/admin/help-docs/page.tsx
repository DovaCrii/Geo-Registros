        ...
        <DetailPanel title="Subir documento" description="Guardamos el archivo y lo publicamos en /ayuda.">
          <form action={uploadHelpDoc} className="grid gap-4 md:grid-cols-2">
            <label className="space-y-2 md:col-span-2">
              ...
              <input name="title" className="w-full rounded-lg border border-slate-300 dark:border-slate-700/80 bg-white dark:bg-slate-900/90 px-4 py-3 text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-accent/20 dark:focus:ring-cyan-500/20" placeholder="Checklist permiso de vuelo" />
            </label>
            <label className="space-y-2">
              <span className="text-xs uppercase tracking-[0.18em] text-slate-500">Categoría</span>
              <input name="category" className="w-full rounded-lg border border-slate-300 dark:border-slate-700/80 bg-white dark:bg-slate-900/90 px-4 py-3 text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-accent/20 dark:focus:ring-cyan-500/20" placeholder="Normativa" />
            </label>
            <label className="space-y-2">
              <span className="text-xs uppercase tracking-[0.18em] text-slate-500">Slug</span>
              <input name="slug" className="w-full rounded-lg border border-slate-300 dark:border-slate-700/80 bg-white dark:bg-slate-900/90 px-4 py-3 text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-accent/20 dark:focus:ring-cyan-500/20" placeholder="checklist-permiso-vuelo" />
            </label>
            <label className="space-y-2 md:col-span-2">
              <span className="text-xs uppercase tracking-[0.18em] text-slate-500">Archivo</span>
              <input name="file" type="file" accept=".pdf,.txt,.docx,.md" className="w-full rounded-lg border border-slate-300 dark:border-slate-700/80 bg-white dark:bg-slate-900/90 px-4 py-3 text-sm text-slate-900 dark:text-slate-100 file:mr-4 file:rounded-md file:border-0 file:bg-slate-100 file:px-3 file:py-1.5 file:text-sm file:font-medium file:text-slate-700 dark:file:bg-slate-800 dark:file:text-slate-200 focus:outline-none focus:ring-2 focus:ring-accent/20 dark:focus:ring-cyan-500/20" />
            </label>
