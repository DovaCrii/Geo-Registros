      ...
      <DetailPanel title="Subir documento" description="Guardamos el archivo y lo publicamos en /ayuda.">
        <form action={uploadHelpDoc} encType="multipart/form-data" className="grid gap-4 md:grid-cols-2">
          ...
          <label className="space-y-2">
            ...
            <input name="category" className="w-full rounded-lg border border-slate-300 bg-white px-4 py-3 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-accent/20 dark:border-slate-700/80 dark:bg-slate-950/90 dark:text-slate-100 dark:placeholder:text-slate-500 dark:focus:ring-cyan-500/20" placeholder="Normativa" />
          </label>
          <label className="space-y-2">
            <span className="text-xs uppercase tracking-[0.18em] text-slate-600 dark:text-slate-400">Slug</span>
            <input name="slug" className="w-full rounded-lg border border-slate-300 bg-white px-4 py-3 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-accent/20 dark:border-slate-700/80 dark:bg-slate-950/90 dark:text-slate-100 dark:placeholder:text-slate-500 dark:focus:ring-cyan-500/20" placeholder="checklist-permiso-vuelo" />
          </label>
          <label className="space-y-2 md:col-span-2">
            ...
            docs.map((doc) => (
              <div key={doc.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-slate-200 dark:border-slate-800/80 bg-white dark:bg-slate-950/45 px-4 py-3 shadow-sm dark:shadow-none">
                ...
                <div className="flex items-center gap-2">
                  <Link href={`/api/help-docs/${doc.id}`} className="rounded-lg border border-slate-300 dark:border-slate-700/80 bg-white dark:bg-slate-950/80 px-3 py-2 text-xs text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800">Descargar</Link>
