    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-start justify-between">
        ...
        {email.status === "failed" && (
          <form action={resendEmail.bind(null, email.id)}>
            <button
              type="submit"
              className="px-4 py-2 rounded-xl bg-cyan-500/20 border border-cyan-500/30 text-cyan-400 text-sm font-medium hover:bg-cyan-500/30 transition-colors"
            >
              Reenviar
            </button>
          </form>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-4">
          <div className="rounded-3xl border border-slate-800/80 bg-slate-950/50 p-5 space-y-4">
            <div>
              <p className="text-xs font-medium text-slate-500 uppercase tracking-wider">Para</p>
              <p className="text-sm text-slate-200 mt-1">{email.to}</p>
            </div>
            <div>
              <p className="text-xs font-medium text-slate-500 uppercase tracking-wider">Enviado</p>
              <p className="text-sm text-slate-200 mt-1">{formatDate(email.sentAt)}</p>
            </div>
            <div>
              <p className="text-xs font-medium text-slate-500 uppercase tracking-wider">Tipo</p>
              <p className="text-sm text-slate-200 mt-1">{TYPE_LABELS[email.type] ?? email.type}</p>
            </div>
            {email.flightPlan && (
              <div>
                ...
                <Link
                  ...
                  className="text-sm text-cyan-400 hover:text-cyan-300 underline underline-offset-2 mt-1 inline-block"
                >
            ...
            {email.error && (
              <div>
                ...
                <p className="text-sm text-red-400 mt-1 font-mono">{email.error}</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="lg:col-span-2">
          <div className="rounded-3xl border border-slate-800/80 bg-slate-950/50 overflow-hidden">
            ...
            <div
              className="p-5 text-sm text-slate-300 leading-relaxed prose prose-invert max-w-none"
